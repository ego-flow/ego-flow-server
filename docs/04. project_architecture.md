...
flowchart TD
    ...
    App -->|POST /auth/app/login| Backend
...
| Worker | recording finalize, technical metadata probe, 3종 output 생성, DB 상태 업데이트 |
...
- repository rename/deactivate/delete는 active stream 또는 finalizing segment 존재 시 차단된다
...
- live 재생: dashboard/Python이 playback ticket을 발급받고 MediaMTX `:8888` native HLS URL을 직접 구성
- processed video 재생: backend signed `/files/*` 경유
...
3. `backend/src/services/repositories.service.ts`
