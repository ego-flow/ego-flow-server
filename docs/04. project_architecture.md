...
| Backend | REST API, JWT 검증, repository access check, MediaMTX auth/webhook 처리 |
| MediaMTX | RTMP ingest, HLS remux, recording file 작성, active path 제공 |
| Redis | active stream session 캐시, BullMQ backend |
| Worker | metadata probe, 3종 output 생성, DB 상태 업데이트 |
| PostgreSQL | 사용자/권한/repository/video/settings 저장 |
...
- stream 등록, publish, active stream, webhook: [09. project_streaming.md](./09.%20project_streaming.md)
...
6. `backend/src/workers/video-processing.worker.ts`
7. `frontend/src/api/*.ts`
8. `frontend/src/routes/repositories.*`, `frontend/src/routes/live.tsx`
