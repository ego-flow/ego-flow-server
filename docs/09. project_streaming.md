...
| GET | `/api/v1/hls-auth?path=...` | `requireAuth` | Caddy `forward_auth`용 internal HLS authorization gate |

여기서 `streamId`는 `RecordingSession.id` 값이다. `requireAuth`는 dashboard session cookie, app JWT, `ef_` static token을 모두 받아준다.
...
sequenceDiagram
    ...
    HP->>BE: GET /hls/live/{repo}/index.m3u8
    BE->>BE: Caddy forward_auth -> GET /api/v1/hls-auth?path=/hls/live/{repo}/index.m3u8
    BE->>Redis: check hlsauth:{sha256(credential)}:{repo} cache
...
1. `path`에서 repository name 추출
2. `sha256(rawCredential)`와 repoName으로 `hlsauth:{hash}:{repo}` cache key 계산
3. Redis cache hit면 즉시 allow
4. miss면 `streamService.findLiveSessionByStreamPath("live/{repo}")`로 활성 session 확인
5. `repositoryService.getRepositoryAccess()`로 read 권한 확인
6. 성공 시 30초 TTL로 cache 저장 후 200 반환

`requireAuth`에서 인증 자체가 실패하면 401, 인증은 됐지만 stream이 없거나 repo read 권한이 없으면 404로 응답한다.
...
| `GET /hls-auth?path=...` | `requireAuth` + 활성 stream 존재 + 해당 repo read 권한 |
