...
sequenceDiagram
    ...
    participant Repo as RepositoriesService
...
sequenceDiagram
    ...
    FE->>FE: HLS URL 조립
    Note over FE: http://{host}:8888/{stream_path}/index.m3u8?ticket={playback_ticket}
    FE->>HP: mount with direct HLS URL
    HP->>MTX: GET /live/{repo}/{recordingSessionId}/index.m3u8?ticket=...
...
backend `liveStreamsService.listLiveStreams()`는:
1. `repositoryAccessService.listAccessibleActiveRepositoryIds()`로 요청자의 accessible repository id set 계산
...
목록 응답은 HLS URL, playback ticket, HTTP upload progress를 반환하지 않는다. client는 `stream_path`와 별도 playback ticket으로 direct HLS URL을 조립한다.

**Step 2 — 선택**

dashboard `LivePage` ([frontend/src/routes/live.tsx](../frontend/src/routes/live.tsx))는 Active Sessions를 MediaMTX / HTTP tab으로 구분한다. 현재 선택된 `recordingSessionId`가 선택된 tab 목록에 없으면 해당 tab의 첫 항목을 선택한다. Python package는 auto-select를 강제하지 않고 caller가 `recordingSessionId`를 선택한다.

`GET /api/v1/live-streams/{recordingSessionId}` 상세 응답은 nullable progress field를 항상 포함한다.

- `bytes_received`
- `last_sequence`
- `last_chunk_at`

HTTP ingest stream이면 Redis upload cache 기준 진행률을 채우고, MediaMTX stream이면 모두 `null`이다.
...
```text
http://{server_host}:8888/{stream_path}/index.m3u8?ticket={playback_ticket}
```

dashboard `HlsPlayer`는 Authorization header를 쓰지 않는다. 대신 hls.js playlist/segment 요청 URL에 initial HLS URL의 `ticket` query를 유지한다.

MediaMTX auth callback은 query `ticket`으로 playback ticket을 받는다.
...
2. 요청 path와 ticket `streamPath` 일치
3. `stream:recording:{recordingSessionId}` 존재 여부
4. live cache `status=STREAMING`
5. live cache repository id와 ticket repository id 일치
...
3. Redis `stream:recording:{recordingSessionId}` cache를 `MGET`
4. cache가 존재하고 접근 가능한 repository에 속한 session만 남김
5. `stream_path`, `playback_available`을 포함한 summary만 응답

이 경로는 MediaMTX API와 recording session DB metadata를 직접 조회하지 않고, Redis active set과 live cache를 조합해 조회한다. HTTP upload 진행률은 선택된 session의 detail endpoint에서만 반환한다.
