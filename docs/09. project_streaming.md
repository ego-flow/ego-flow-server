...
3. ticket `ingestType=MEDIAMTX`
4. 요청 path와 ticket `streamPath` 일치
5. `stream:recording:{recordingSessionId}` 존재 여부
6. live cache `status=STREAMING`, `ingestType=MEDIAMTX`
7. live cache repository id와 ticket repository id 일치
