...
- `stream:ticket:{ticketId}`: publish-ticket 발급 직후 60초. stream-ready consume 시 consumed 상태로 저장하면서 다시 60초로 재설정한다
- `stream:recording:{recordingSessionId}`: register 직후 PENDING cache는 5분, stream-ready 이후 live cache는 24시간
...
- `STREAMING` 또는 `STOP_REQUESTED`인데 MediaMTX path가 사라지면 `FINALIZING`
...
`ego-flow-server/mediamtx-hooks/` 아래의 wrapper script가 MediaMTX env를 JSON body로 변환해서 backend에 전달한다. `recording-segment-create`와 `recording-segment-complete`는 `source_id`가 비어 있거나 아예 전달되지 않을 수 있으므로, backend는 stream path의 recordingSessionId를 기준으로 세션을 복구한다.
...
sequenceDiagram
    ...
    Hooks->>BE: JSON POST
    BE->>BE: path의 recordingSessionId / segment mapping 확인
...
- `stream-not-ready`: stream path의 recordingSessionId가 가리키는 session을 FINALIZING으로 전환한다
- `recording-segment-create`: stream path의 recordingSessionId로 세션을 찾고 `segment:{segmentPath}`를 저장한다
- `recording-segment-complete`: 우선 `segment:{segmentPath}`를 사용하고, create hook이 누락된 경우에는 stream path의 recordingSessionId로 복구한다
- segment mapping과 stream path 복구가 모두 실패하면 잘못 귀속하지 않고 no-op + 경고 로그로 끝낸다
