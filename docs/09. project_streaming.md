...
- 오래된 `PENDING` 예약은 reconcile loop가 닫지 않는다. publish-ticket은 Redis PENDING cache가 살아 있는 경우에만 발급된다
...
- register 후 5분 내 publish-ticket이 없으면 Redis PENDING cache가 만료된다. DB의 `PENDING` row는 CLOSED로 바꾸지 않는다
