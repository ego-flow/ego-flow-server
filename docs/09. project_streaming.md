...
4. MediaMTX HLS read/playback auth callback에서 Redis ticket 검증
...
| POST | `/api/v1/auth/mediamtx` | MediaMTX internal | publish auth와 HLS read/playback auth를 action/protocol 기준으로 분기 |
...
sequenceDiagram
    ...
    BE-->>FE: playback_ticket, playback_ticket_expires_at
    FE->>FE: HLS URL 조립
    Note over FE: http://{host}:8888/{stream_path}/index.m3u8?ticket={playback_ticket}
    FE->>HP: mount with direct HLS URL + Authorization Bearer playback_ticket
    HP->>MTX: GET /live/{repo}/{recordingSessionId}/index.m3u8?ticket=...
    MTX->>BE: POST /api/v1/auth/mediamtx action=read/playback protocol=hls
...
dashboard `HlsPlayer`는 hls.js XHR마다 아래 header도 붙인다.

```text
Authorization: Bearer {playback_ticket}
```

MediaMTX auth callback은 `Authorization` token, query `ticket`, query `token`, password 순으로 playback ticket을 추출한다.

**Step 5 — MediaMTX HLS read/playback auth**

MediaMTX는 HLS playlist와 segment 요청마다 `POST /api/v1/auth/mediamtx`를 호출한다. backend는 `action=read|playback` 및 `protocol=hls` 조합을 HLS playback auth로 처리한다.

hot path 검증은 Redis만 사용한다.

1. `stream:hls-ticket:{ticketId}` 존재 여부와 `status=active`
2. ticket `ingestType=MEDIAMTX`
3. 요청 path와 ticket `streamPath` 일치
4. `stream:recording:{recordingSessionId}` 존재 여부
5. live cache `status=STREAMING`, `ingestType=MEDIAMTX`
...
| MediaMTX HLS read/playback | Redis playback ticket + Redis active live cache |
...
- `[hls-auth] allowed` / `[hls-auth] denied`: MediaMTX HLS read/playback auth allow/deny 및 사유
