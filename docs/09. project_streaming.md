...
sequenceDiagram
    ...
    MTX->>BE: POST /api/v1/auth/publish action=publish
...
즉 HLS 재생 시점에는 MediaMTX가 backend `/api/v1/auth/publish`를 다시 호출하지 않는다.
`/api/v1/auth/publish`는 publish ticket 검증만 맡고, HLS playback은 Caddy `forward_auth`가 유일한 게이트다.
...
- `[publish-auth] allowed` / `[publish-auth] denied`: publish 시 MediaMTX authHTTPAddress hook의 allow/deny 및 사유
