...
- register만 하고 publish하지 않으면 `PENDING` timeout 후 `CLOSED + REGISTRATION_TIMEOUT`으로 정리된다
...
| `stream:recording:{recordingSessionId}` | register 직후 PENDING cache 및 stream-ready 이후 live metadata JSON |
| `segment:{segmentPath}` | segment file path에서 recordingSessionId를 복원하기 위한 short-lived mapping |
...
- `stream:recording:{recordingSessionId}`: register 직후 PENDING cache는 5분, stream-ready 이후 live cache는 2시간
- `segment:{segmentPath}`: 2시간
...
2. Redis `stream:recording:{recordingSessionId}` live/pending cache, active set 및 segment mapping
...
- 오래된 `PENDING` 예약은 reconcile loop가 `CLOSED + REGISTRATION_TIMEOUT`으로 정리한다
- `STREAMING`인데 MediaMTX path가 사라졌으면 `CLOSED + closedAt/endReason`으로 보정하고, segment complete 확정 이후 `RecordingSegment` 후처리를 시작한다
- live session 목록 조회는 Redis active set으로 후보 id를 좁힌 뒤 `stream:recording:{recordingSessionId}` metadata를 조회한다
- `stream-not-ready`와 segment create는 stream path의 recordingSessionId를 사용한다. segment complete는 `segment:{segmentPath}` mapping을 우선 사용하고, create hook이 누락된 경우 stream path로 복구한다
...
- Python: list를 받은 뒤 caller가 사용할 stream의 `hls_path`를 명시적으로 선택
...
2. 선택된 stream에 대한 HLS/WHEP path 제공
3. Caddy가 각 HLS 요청에서 수행하는 `forward_auth` 권한 검증

### Endpoints

| Method | Path | Auth | 역할 |
| --- | --- | --- | --- |
| GET | `/api/v1/live-streams` | `requireDashboardOrAppOrPython` | 접근 가능한 active stream 목록 + `hls_path`, `whep_path` |
...
sequenceDiagram
    ...
    loop every 2s
        ...
        BE->>Redis: active set 조회
        BE->>Redis: stream:recording:{recordingSessionId} metadata 조회
...
4. `status === "STREAMING"`인 cache만 남김
...
목록 조회는 stream metadata 기준으로 DB/MediaMTX를 직접 조회하지 않는 Redis read-only 경로다.
...
- `[startup] runtime playback config`: 부팅 시 `rtmpPort`, `rtmpsPort`, `HLS_PATH_PREFIX`, 고정 MediaMTX API URL
...
- `register createdAt -> CLOSED(registration timeout)`
...
- register 후 5분 내 publish auth가 없으면 `CLOSED + REGISTRATION_TIMEOUT`
- publish-ticket가 발급됐지만 registration timeout 안에 `stream-ready`가 오지 않으면 `CLOSED + REGISTRATION_TIMEOUT`
- `STREAMING`인데 MediaMTX path가 사라지면 `CLOSED + closedAt/endReason`으로 보정한 뒤 segment complete를 기다림
- enqueue가 누락된 `RecordingSegment.status = WRITE_DONE` row는 reconcile loop가 finalize job을 재시도
...
`ego-flow-server/mediamtx-hooks/` 아래의 wrapper script가 MediaMTX env를 JSON body로 변환해서 backend에 전달한다. `stream-ready`는 `path`, `query`, `ticket`만 전달하며 source id/type을 사용하지 않는다. `recording-segment-create`와 `recording-segment-complete`는 `source_id`가 비어 있거나 아예 전달되지 않을 수 있으므로, backend는 stream path의 recordingSessionId를 기준으로 세션을 복구한다.
...
sequenceDiagram
    ...
    Hooks->>BE: JSON POST
    BE->>BE: path의 recordingSessionId / segment session mapping 확인
...
- `recording-segment-create`: stream path의 recordingSessionId로 세션을 찾고 session 단일 `RecordingSegment`를 `WRITING`으로 만든 뒤 `segment:{segmentPath}`에 recordingSessionId를 저장한다
- `recording-segment-complete`: 우선 `segment:{segmentPath}`를 사용하고, create hook이 누락된 경우에는 stream path의 recordingSessionId로 복구한다
...
- segment session mapping과 stream path 복구가 모두 실패하면 잘못 귀속하지 않고 no-op + 경고 로그로 끝낸다
...
- `stream-not-ready`는 stream path의 recordingSessionId로 세션을 찾는다. segment complete는 Redis mapping을 우선 사용하되, mapping miss 시 stream path의 recordingSessionId로 복구한다
