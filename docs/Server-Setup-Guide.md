...
| --- | --- | --- | --- |
| `22` | TCP | Yes | SSH 접속 |
| `80` 또는 `PUBLIC_HTTP_PORT` | TCP | Yes | Dashboard, Backend API, Swagger UI, HLS, WHIP HTTP ingress |
| `1935` | TCP | RTMP 사용 시 Yes | RTMP publish ingest |
| `1936` | TCP | RTMPS 사용 시 Yes | RTMPS publish ingest |
| `8189` | UDP | WebRTC/WHIP 사용 시 Yes | MediaMTX WebRTC ICE/UDP media |
...
| `8888` | MediaMTX HLS internal port |
| `8889` | MediaMTX WHIP/WebRTC internal HTTP port, Caddy가 public HTTP port에서 proxy |
| `9997` | MediaMTX control API internal port |
| `5432` | PostgreSQL internal port |
| `6379` | Redis internal port |

기본 Caddy 설정은 plain HTTP `:80`만 사용한다. HTTPS가 필요하면 서버 앞단에 별도 TLS reverse proxy나 load balancer를 두고, 그 경우 `443/TCP`를 추가로 열어야 한다.
...
| `PUBLIC_HTTP_PORT` | `80` | Caddy public HTTP port |
...
정상적인 경우 Docker, Compose, HTTP/RTMP/RTMPS port, HLS internal port, data root, config/env file 경로가 출력된다.
...
`PUBLIC_HTTP_PORT`, `1935`, `1936`, `8189/udp`를 이미 다른 프로세스가 사용 중이면 stack이 올라오지 않는다. 기존 프로세스를 종료하거나 `PUBLIC_HTTP_PORT`만 다른 값으로 바꾼다.

RTMP/RTMPS/WebRTC port는 현재 고정 계약이므로 config로 변경하지 않는다.
