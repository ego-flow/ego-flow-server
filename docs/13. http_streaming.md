...
```json
{
  ...
  "whip_publish_url": "http://server/live/myrepo/2b42c60f-8e94-4c85-933f-182c6496e620/whip?ticket=opaque-ticket"
}
```
...
- WHIP endpoint는 MediaMTX native path(`/live/{repo}/{recordingSessionId}/whip`)를 그대로 노출한다. MediaMTX가 `Location: /live/{repo}/{recordingSessionId}/whip/{sessionId}`를 반환하므로, 클라이언트의 후속 `PATCH`/`DELETE`도 같은 public path로 동작한다.
...
| `PUBLIC_WHIP_BASE_URL` | `http://127.0.0.1:${PUBLIC_HTTP_PORT}/live` | 외부 publish URL base. 최종 URL은 `{base}/{repo}/{recordingSessionId}/whip?ticket=...` (HTTPS 운영 시 `https://.../live` 로 override) |
...
| Key | Default | 설명 |
| --- | --- | --- |
| `PUBLIC_WHEP_BASE_URL` | `http://127.0.0.1:${PUBLIC_HTTP_PORT}/live` | client에 advertise하는 WHEP URL base. 현재 응답은 origin-relative `/live/{repo}/{recordingSessionId}/whep`를 사용하므로 옵셔널 |
