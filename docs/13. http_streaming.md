...
- MediaMTX read/playback auth callback은 `/api/v1/auth/mediamtx`가 처리하며 Redis `stream:hls-ticket:{ticketId}`, `query.user_id`, `stream:recording:{recordingSessionId}`만 검증한다.
