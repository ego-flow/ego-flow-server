...
flowchart LR
    ...
    Viewer -->|HLS native /live/{repo}/{recordingSessionId}/index.m3u8?ticket=...| MediaMTX
...
- `HTTP` stream은 `playback_available=false`이고 upload progress만 표시한다.
...
- HLS URL 형식은 `http://{host}:8888/{stream_path}/index.m3u8?ticket={playback_ticket}`이다.
- MediaMTX read/playback auth callback은 `/api/v1/auth/mediamtx`가 처리하며 Redis `stream:hls-ticket:{ticketId}`와 `stream:recording:{recordingSessionId}`만 검증한다.
...
- HLS read/playback 실패: backend log에서 `[hls-auth] denied` prefix와 reject reason을 확인.
...
2. MediaMTX WebRTC 설정
   ...
   - `authHTTPExclude`에는 `api`만 두고 publish/read/playback은 제외하지 않는다.
...
6. smoke check
   ...
   - HLS playback URL은 `http://{host}:8888/{stream_path}/index.m3u8?ticket={playback_ticket}` 형식.
