...
RecordingSession 모델, ticket/owner lease, hook lifecycle, finalize 흐름은 RTMP와 완전히 동일하다.
...
flowchart LR
    ...
    App -->|WHIP POST /live/{repo}/whip?ticket=...| Caddy
    Viewer["Dashboard / Python"] -->|WHEP /live/{repo}/whep| Caddy
    Caddy -->|proxy native /live/{repo}/whip| MediaMTX
    Caddy -->|forward_auth /api/v1/whep-auth| Backend
    Caddy -->|proxy native /live/{repo}/whep| MediaMTX
...
sequenceDiagram
    ...
    BE-->>App: { recording_session_id, repository_name, stream_path }
    App->>BE: POST /streams/{recordingSessionId}/publish-ticket
    BE-->>App: { publish_ticket, rtmp_publish_base_url, whip_publish_url }
    App->>Caddy: POST /live/{repo}/whip?ticket=... (SDP offer)
    Caddy->>MTX: proxy -> POST /live/{repo}/whip
    ...
    BE->>BE: owner publishing refresh + STREAMING 승격
```

### 3.2 publish-ticket 응답

`POST /api/v1/streams/{recordingSessionId}/publish-ticket` 응답에 RTMP base와 함께 **WHIP URL이 한 줄 추가**된다.
...
```json
{
  ...
  "stream_path": "live/myrepo",
  "connection_id": "...",
  "generation": 1,
  "publish_ticket": "opaque-ticket",
  "publish_ticket_expires_at": "...",
  "rtmp_publish_base_url": "rtmp://server:1935/live",
  "whip_publish_url": "http://server/live/myrepo/whip?ticket=opaque-ticket"
}
```

- 같은 ticket로 RTMP 또는 WHIP 중 **하나만** 골라 publish하면 된다. 둘 다 동시에 publish하면 `overridePublisher` 정책에 따라 나중 publisher가 takeover한다.
- ticket lease/heartbeat/generation 의미는 RTMP와 동일하다. WHIP publish가 성공하면 `POST /api/v1/streams/{recordingSessionId}/connections/{connectionId}/heartbeat`로 5초 간격 heartbeat를 보내야 한다.
- WHIP endpoint는 MediaMTX native path(`/live/{repo}/whip`)를 그대로 노출한다. MediaMTX가 `Location: /live/{repo}/whip/{sessionId}`를 반환하므로, 클라이언트의 후속 `PATCH`/`DELETE`도 같은 public path로 동작한다.

### 3.3 인증

- MediaMTX `authHTTPAddress`는 RTMP/WebRTC 구분 없이 동일한 `POST /api/v1/auth/rtmp`를 호출하고 `protocol` 필드만 다르다.
- backend는 `query`에서 `ticket=`만 보고 ticket id를 추출해 publish ticket + owner lease를 검증한다.
...
| Key | Default | 설명 |
| --- | --- | --- |
| `WEBRTC_PORT` (`config.json`) | `8889` | MediaMTX WebRTC signaling port (internal) |
| `WEBRTC_UDP_PORT` (compose env) | `8189` | host에 expose하는 WebRTC ICE UDP port |
| `PUBLIC_WHIP_BASE_URL` | `http://127.0.0.1:${PUBLIC_HTTP_PORT}/live` | 외부 publish URL base. 최종 URL은 `{base}/{repo}/whip?ticket=...` (HTTPS 운영 시 `https://.../live` 로 override) |
...
sequenceDiagram
    ...
    Viewer->>Caddy: POST /live/{repo}/whep (SDP offer)
    Caddy->>BE: forward_auth GET /api/v1/whep-auth?path=/live/{repo}/whep
    BE-->>Caddy: 200 (read 권한 OK)
    Caddy->>MTX: proxy -> POST /live/{repo}/whep
...
```json
{
  ...
  "registered_at": "...",
  "status": "live",
  "hls_path": "/hls/live/myrepo/index.m3u8",
  "whep_path": "/live/myrepo/whep"
}
```
...
WHEP endpoint도 MediaMTX native path(`/live/{repo}/whep`)를 그대로 노출한다. MediaMTX가 `Location: /live/{repo}/whep/{sessionId}`를 반환하므로, 클라이언트의 후속 `PATCH`/`DELETE`도 같은 public path로 동작한다.

### 4.3 인증

- WHEP gate는 HLS gate와 1:1로 같은 패턴이다.
- Caddy가 `/live/{repo}/whep`와 `/live/{repo}/whep/{sessionId}` 요청에 대해 backend `GET /api/v1/whep-auth?path=...`를 forward_auth subrequest로 호출한다.
- `whepAuthService.authorize()`는 다음 순서로 동작한다.
  1. path에서 repository name 추출 (`/live/{repo}/whep` 또는 `/live/{repo}/whep/{sessionId}`)
  2. `sha256(rawCredential)` × `repoName`으로 `whepauth:{hash}:{repo}` cache key 계산
...
| Key | Default | 설명 |
| --- | --- | --- |
| `PUBLIC_WHEP_BASE_URL` | `http://127.0.0.1:${PUBLIC_HTTP_PORT}/live` | client에 advertise하는 WHEP URL base. 현재 응답은 origin-relative `/live/{repo}/whep`를 사용하므로 옵셔널 |
...
ports:
  - "${RTMP_PORT:-1935}:1935"
  - "${RTMPS_PORT:-1936}:1936"
  - "${WEBRTC_UDP_PORT:-8189}:8189/udp"
```
...
## 7. RTMP / WHIP 동시 운영 정책

- RecordingSession 1개에 대해 동시에 활성 publisher는 1명이다.
- `overridePublisher: yes` 설정에 의해 새로 들어온 publisher가 기존을 takeover한다. publish-ticket을 새로 받으면 generation이 증가하므로, 기존 RTMP publisher가 살아 있어도 새 WHIP publisher가 이기는 식의 경합이 가능하다.
- 운영 권장: 클라이언트는 환경에 따라 **하나만 선택**해 사용한다 (예: 글래스 = RTMP, 모바일 = WHIP). 동일 RecordingSession을 RTMP/WHIP로 교대 publish하는 시나리오는 명시적으로 지원하지 않는다.
...
- `whip_publish_url`은 단일 URL로 표현되므로, 동일 server에 IPv4/IPv6 dual stack을 advertise하지는 않는다. 필요하면 `PUBLIC_WHIP_BASE_URL`을 정확한 호스트로 override한다.
- ticket scope는 protocol-agnostic이지만, 한 RecordingSession 당 동시에 둘 이상의 protocol에서 publish하는 시나리오는 지원하지 않는다 (마지막 publisher가 takeover).
...
1. `PUBLIC_WHIP_BASE_URL`은 클라이언트가 접근하는 HTTPS origin 기준으로 설정한다. 예: `https://stream.example.com/live`
2. `WEBRTC_UDP_PORT` 또는 실제 host UDP `8189` inbound를 방화벽/보안 그룹에서 허용한다.
