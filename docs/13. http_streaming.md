...
- `GET /api/v1/live-streams` 응답은 `ingest_type`, `playback_available`, `hls_path`를 제공한다.
- `MEDIAMTX` stream은 `playback_available=true`, `hls_path=/hls/...`이다.
- `HTTP` stream은 `playback_available=false`, `hls_path=null`이고 upload progress만 표시한다.
...
HTTP upload 상세 sequence diagram은 [`.mermaid/http-streaming.mmd`](../.mermaid/http-streaming.mmd)를 참고한다.
