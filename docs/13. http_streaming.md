...
상세 동작 원리는 [`../.mermaid/whep-playback.mmd`](../.mermaid/whep-playback.mmd)에 별도 sequence diagram으로 정리한다.
...
WHIP/WHEP를 실제 운영 publish/playback 경로로 켜기 전에 아래 항목을 확인한다.

1. Caddy route
   - `/live/{repo}/{recordingSessionId}/whip`와 하위 `Location` path가 `mediamtx:8889`로 proxy되어야 한다.
   - `/live/{repo}/{recordingSessionId}/whep`와 하위 `Location` path는 `GET /api/v1/whep-auth?path=...` forward_auth 이후 `mediamtx:8889`로 proxy되어야 한다.
...
6. smoke check
   ...
   - publish는 성공하지만 media가 안 보이면 MediaMTX log의 WebRTC/ICE 메시지, UDP `8189`, `webrtcAdditionalHosts`, TURN 필요 여부를 순서대로 확인.
