# EgoFlow Server HTTP Streaming (WHIP)

이 문서는 `ego-flow-server`에 추가된 **HTTP 기반 publish 경로**를 정리한 문서다.
기존 RTMP publish + HLS playback 흐름은 [`09. project_streaming.md`](./09.%20project_streaming.md)를 그대로 따르고, 이 문서는 그 위에 얹힌 **WHIP(WebRTC-HTTP Ingestion Protocol) publish** 경로만 다룬다.

요약:

- Publish: **WHIP** — RTMP의 대체 publish 경로로 같은 publish ticket를 그대로 사용한다.
- Playback: **HLS only** — live playback은 기존 HLS 경로만 유지한다.

RecordingSession 모델, publish ticket, hook lifecycle, finalize 흐름은 RTMP와 동일하다.
달라지는 것은 **publish transport와 ingress URL**뿐이다.
...
flowchart LR
    ...
    Viewer["Dashboard / Python"] -->|HLS /hls/live/{repo}/{recordingSessionId}/index.m3u8| Caddy
    Caddy -->|proxy native /live/{repo}/{recordingSessionId}/whip| MediaMTX
    Caddy -->|forward_auth /api/v1/hls-auth| Backend
    Caddy -->|proxy /hls/* to MediaMTX HLS| MediaMTX
    MediaMTX -->|HTTP auth /api/v1/auth/publish| Backend
...
- **HLS playback**: Caddy `forward_auth` -> backend `/api/v1/hls-auth` -> MediaMTX `:8888`.
...
| --- | --- | --- | --- |
| `1935` | TCP | host | RTMP publish |
| `1936` | TCP | host | RTMPS publish (optional) |
| `80/443` | TCP | host (Caddy) | HLS / WHIP / API |
...
sequenceDiagram
    ...
    MTX->>BE: POST /api/v1/auth/publish (protocol=webrtc, action=publish, query="ticket=...")
...
- MediaMTX `authHTTPAddress`는 RTMP/WebRTC 구분 없이 동일한 `POST /api/v1/auth/publish`를 호출하고 `protocol` 필드만 다르다.
...
- `GET /api/v1/live-streams` 응답은 `hls_path`만 제공한다.
- Dashboard live tab은 `hls_path`를 `HlsPlayer`에 전달한다.
- HLS 권한 검증은 Caddy `forward_auth -> GET /api/v1/hls-auth?path=...`가 담당한다.
- MediaMTX read/playback auth callback은 `mediamtx.yml`의 `authHTTPExclude`로 제외되어 있다.

상세 HLS playback 흐름은 [`.mermaid/hls-playback.mmd`](../.mermaid/hls-playback.mmd)를 참고한다.
...
Playback은 HLS route(`/hls/*`)만 사용한다.
...
- HLS gate 실패: backend access log에서 `GET /api/v1/hls-auth` 404 응답을 확인.
...
2. MediaMTX WebRTC 설정
   ...
   - `webrtcAddress: :8889`
   - `webrtcLocalUDPAddress: :8189`
   - `authHTTPAddress: http://backend:3000/api/v1/auth/publish`
   - `authHTTPExclude`에는 read/playback/api만 두고 publish는 제외하지 않는다.
