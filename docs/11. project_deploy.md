...
- `./scripts/run.sh`: 현재 local Docker Compose stack의 유일한 사용자-facing 진입점
...
| 구분 | 목적 | 기준 자산 |
| --- | --- | --- |
| Local deployment | 개발/테스트/로컬 재현 | `ego-flow-server/compose.yml`, `ego-flow-server/compose.local.yml`, `ego-flow-server/mediamtx.yml`, `ego-flow-server/Caddyfile`, `ego-flow-server/scripts/run.sh` |
...
HTTP 접근은 `PUBLIC_HTTP_PORT` 하나로 통일된다.

- `PUBLIC_HTTP_PORT=80`이면 `http://localhost`
- `PUBLIC_HTTP_PORT`를 바꿨다면 `http://localhost:{PUBLIC_HTTP_PORT}`

이 HTTP base URL 기준으로 확인할 주소:
...
- RTMPS ingest: `rtmps://localhost:{RTMPS_PORT}/live` (`RTMPS_ENCRYPTION_MODE != no`일 때만 사용)
...
- `.env`: `ADMIN_DEFAULT_PASSWORD`, `JWT_SECRET`는 필수이고, `DATABASE_URL`, `REDIS_URL`, `HF_TOKEN`, public URL override는 필요할 때만 덮어쓴다
...
다른 프로세스가 `PUBLIC_HTTP_PORT`, `RTMP_PORT`, `RTMPS_PORT`, `HLS_PORT`를 사용 중이면 Compose가 실패할 수 있다.
...
| `RTMPS_ENCRYPTION_MODE` | MediaMTX RTMP encryption mode (`no`, `optional`, `strict`) |
| `RTMPS_CERT_PATH` | MediaMTX RTMPS certificate path |
| `RTMPS_KEY_PATH` | MediaMTX RTMPS private key path |
...
- production override는 `${CONFIG_ROOT}/certs:/certs:ro`를 mount해서 RTMPS cert/key 전달 경로를 제공한다
...
3. `config.json`에서 `PUBLIC_HTTP_PORT`, `RTMP_PORT`, `RTMPS_PORT`, `HLS_PORT`를 읽어 compose env로 export
...
- RTMPS: `rtmps://<EC2_HOST>:<RTMPS_PORT>/live` (`RTMPS_ENCRYPTION_MODE != no`일 때)

RTMP/HLS 상태를 확인할 때는 backend 로그에서 다음 prefix를 우선 본다.
...
#### streaming hook / RTMPS 운영 메모

- hook endpoint는 external client용 API가 아니다. `backend:3000` 내부 네트워크로만 접근된다는 전제를 유지해야 한다.
- RTMPS는 준비되어 있지만 기본 cutover는 아니다. `RTMPS_ENCRYPTION_MODE=no`이면 앱과 운영 문서는 계속 `rtmp://.../live`를 기준으로 둔다.
- RTMPS를 실제로 켤 때는 cert/key 배치, `RTMPS_ENCRYPTION_MODE`, `PUBLIC_RTMP_BASE_URL=rtmps://.../live`, published `RTMPS_PORT`를 함께 점검해야 한다.
- RTMPS를 켠다고 hook endpoint 보안이 자동으로 강화되지는 않는다. shared secret/HMAC은 후속 hardening 항목이다.
...
2. `./scripts/run.sh up`
3. `http://localhost/api/v1/health` 또는 `http://localhost:<PUBLIC_HTTP_PORT>/api/v1/health`
4. `http://localhost/api-docs` 또는 `http://localhost:<PUBLIC_HTTP_PORT>/api-docs`
5. `http://localhost` 또는 `http://localhost:<PUBLIC_HTTP_PORT>`
...
## 5. RTMP Cutover Checklist

Task5 기준 RTMP/RTMPS cutover는 backend와 Android 앱을 같은 변경 집합으로 배포하는 single cutover를 기본으로 한다.
...
2. Android 앱 설치
3. plain RTMP 검증
4. heartbeat/reconnect 검증
5. RTMPS 검증

배포 직후 우선 확인:

1. `docker compose --env-file /opt/egoflow/config/.env.compose -f ego-flow-server/compose.yml -f ego-flow-server/compose.prod.yml logs -f backend mediamtx proxy`
2. backend 로그에서 `[startup] runtime playback config`
3. backend 로그에서 `[rtmp-register]`, `[rtmp-ticket]`, `[rtmp-auth]`, `[rtmp-state]`, `[rtmp-owner]`
4. 앱 diagnostics에서 `backend.register.succeeded`, `backend.ticket.succeeded`, `transport.publish_connected`, `backend.heartbeat.refreshed`

### 5.1 Plain RTMP smoke

1. 앱에서 streaming 시작
2. backend에 `[rtmp-ticket] issued`, `[rtmp-auth] allowed`, `[rtmp-state] pending-to-streaming`가 순서대로 나오는지 확인
3. 앱 diagnostics에서 같은 `recordingSessionId`, `connectionId`, `generation`이 보이는지 확인
4. `/live` 또는 HLS URL에서 playback이 뜨는지 확인
5. stop 후 `[rtmp-state] stop-requested`, `[rtmp-state] stream-to-finalizing`, `[rtmp-finalize] enqueued`를 확인

### 5.2 Heartbeat / reconnect smoke

1. publish 중 네트워크를 잠시 끊는다
2. 앱 diagnostics에서 `backend.heartbeat.failed`, `transport.reconnect_scheduled`를 확인한다
3. reconnect 뒤 같은 `recordingSessionId`에 대해 새 `connectionId`, 새 `generation`이 발급되는지 확인한다
4. backend에서 old generation cleanup이 current owner를 지우지 않는지 `[rtmp-owner] generation-mismatch`, `[rtmp-owner] heartbeat-refreshed`로 확인한다

### 5.3 RTMPS smoke

전제:

- `/opt/egoflow/config/.env`에 `RTMPS_ENCRYPTION_MODE`, `RTMPS_CERT_PATH`, `RTMPS_KEY_PATH`, `PUBLIC_RTMP_BASE_URL=rtmps://.../live`
- cert/key mount가 실제로 존재

확인:

1. 앱 diagnostics에서 `publishUrl=rtmps://...`
2. `transport.connecting`, `transport.publish_started`, `transport.publish_connected`
3. backend에서 plain RTMP와 같은 auth/state progression

### 5.4 Redis fail-closed smoke

1. publish 중 Redis를 재시작한다
2. heartbeat가 `OWNER_LEASE_MISSING` 또는 stale 계열 오류로 실패하는지 확인한다
3. 현재 publish attempt가 종료되고, 새 publish-ticket 기반 reconnect만 허용되는지 확인한다
4. 오래된 owner/ticket metadata를 투명 복구하려는 동작이 없는지 확인한다

## 6. Rollback

RTMP Task1~5 cutover rollback은 "이전 backend/app 조합으로 되돌리고, 진행 중 publish는 중단된다"를 전제로 한다.

최소 순서:

1. 새 배포에서 active publish가 없는지 확인한다
2. 이전 release metadata와 image SHA를 고른다
3. `/opt/egoflow/config`를 이전 snapshot으로 되돌린다
4. 이전 backend/dashboard image SHA로 재배포한다
5. Android 앱도 이전 버전으로 되돌린다
6. `deploy/ec2/deploy.sh smoke-test`와 plain RTMP smoke를 다시 수행한다

주의:

- Redis owner/ticket/connection metadata는 fail-closed이므로 rollback 중 in-flight publish를 유지하는 방식은 기대하지 않는다.
- hook endpoint는 internal-only 전제다. public routing 변경으로 우회 복구하지 않는다.
