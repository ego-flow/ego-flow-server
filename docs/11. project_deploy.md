...
- `down`: 컨테이너 종료 및 제거
- `reset`: 컨테이너/볼륨 삭제 후 `data/redis`, `data/raw`, `data/datasets` bind mount 정리
...
- MediaMTX는 `host.docker.internal:3000`을 통해 backend auth/hook에 연결된다
...
현재는 PoC 단계이므로, schema가 바뀌면 기존 데이터를 보존한 migration보다 전체 reset 후 재기동을 권장한다.
...
이 방식은 PostgreSQL volume과 raw/generated/redis bind mount를 함께 정리한 뒤, migration과 seed를 처음부터 다시 적용한다.
...
remote MediaMTX도 현재 local과 동일한 lifecycle hook 구조를 사용한다.
...
차이는 local은 `mediamtx-hooks/` wrapper script를 mount해서 쓰고, remote는 `deploy/ec2/mediamtx.yml`에 inline `wget --post-data`를 직접 둔다는 점이다.
...
현재는 PoC 단계이므로 운영 환경도 기존 데이터를 유지한 migration보다 전체 삭제 후 재기동을 권장한다.

예시 절차:

```bash
cd /opt/egoflow/repo/deploy/ec2
docker compose --env-file .env.prod -f docker-compose.yml down -v --remove-orphans
...
이 방식은 기존 Postgres/Redis/raw/generated 데이터를 모두 비우고, 새 schema 기준으로 컨테이너를 다시 올린다.

주의:

- `down` 단계에서도 반드시 `--env-file .env.prod -f docker-compose.yml`를 붙여야 한다
- 그렇지 않으면 `POSTGRES_USER`, `BACKEND_IMAGE`, `DATA_ROOT` 등이 비어 invalid compose project 오류가 날 수 있다
