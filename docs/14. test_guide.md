...
| GHCR (`ghcr.io/dennis0405`) | backend / worker / dashboard image registry |
...
1. host PC에서 최신 코드로 backend / worker / dashboard 이미지 빌드 후 GHCR push.
...
ghcr.io/dennis0405/ego-flow-server-worker:latest
ghcr.io/dennis0405/ego-flow-server-dashboard:latest
```

> backend와 worker는 동일한 이미지에 다른 tag를 붙인 것이다. (`build-registry-images.sh`가 `docker tag`로 alias만 추가)
...
- WHIP publish / WHEP play: `http://13.209.88.203/live/{repo}/whip|whep`
...
| RTMP publish 실패(`[rtmp-auth] denied`) | publish-ticket 만료 / repository 권한 / `PUBLIC_RTMP_BASE_URL` 불일치 확인 |
