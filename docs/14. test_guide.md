...
- Open inbound ports
  - `22/tcp` — SSH
  - `80/tcp` — HTTP (Caddy proxy, dashboard / API / WHIP)
  - `8888/tcp` — direct HLS playback
...
flowchart LR
    ...
    Client["Test client (app / browser)"] -->|HTTP / RTMP / WHIP| EC2
```
...
- `--vite-backend-origin http://13.209.88.203` — dashboard SPA가 사용하는 backend origin. RTMP/WHIP URL 등 absolute URL 생성에 사용.
...
- HLS playback: `http://13.209.88.203:8888/live/{repo}/{recordingSessionId}/index.m3u8?ticket={playback_ticket}&user_id={viewer_user_id}`
- WHIP publish: `http://13.209.88.203/live/{repo}/{recordingSessionId}/whip`

> WHIP를 실제로 사용할 경우 ICE media용 UDP `8189`가 EC2 보안 그룹에 inbound로 열려 있어야 한다. 현재는 RTMP/RTMPS만 우선 검증 중이고, WHIP 사용 시점에 보안 그룹과 `webrtcAdditionalHosts` 설정을 함께 점검한다. 자세한 내용은 [13. http_streaming.md](./13.%20http_streaming.md) §10 참고.
...
- HLS playback base: `http://13.209.88.203:8888`
...
| RTMP publish 실패(`[publish-auth] denied`) | publish-ticket 만료 / repository 권한 / app이 조립한 RTMP URL의 host/path/ticket 확인 |
