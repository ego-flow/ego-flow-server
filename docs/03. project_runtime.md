...
| `mediamtx` | `1935`, `1936`, `HLS_PORT`, `9997` (internal) | RTMP ingest, optional RTMPS ingest, HLS, Control API |
...
- `/hls/*`를 MediaMTX HLS output으로 reverse proxy 가능
...
- live/playback API가 반환하는 HLS 메타데이터는 absolute URL이 아니라 origin-relative path(`/hls/...`)다.
- dashboard, python package, app은 각자의 API origin 기준으로 이 HLS path를 resolve해야 한다.
