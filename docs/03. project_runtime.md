...
- `./scripts/run.sh up`은 현재 target root를 `.run/target-directory`에 기록한다. 이후 `TARGET_DIRECTORY`가 바뀌면 이 값을 이전 host data root로 사용해 migration을 시도하고, state가 없을 때만 legacy `./data` 또는 `/opt/egoflow/data`를 fallback으로 본다.
