...
- 다른 장치가 접속하는 서버에서는 `.env`의 `PUBLIC_RTMP_BASE_URL`, `PUBLIC_HLS_BASE_URL`를 실제 서버에서 외부 클라이언트가 도달 가능한 주소로 설정해야 한다.
- 운영에서는 HLS 전용 포트를 별도로 열지 않는 경우 `PUBLIC_HLS_BASE_URL=http://<host>/hls`처럼 proxy path를 사용하는 편이 안전하다.
