...
| `mediamtx` | `RTMP_PORT`, `HLS_PORT`, `9997` (internal) | RTMP ingest, HLS, Control API |
...
- 현재 기본 로컬 실행 경로는 `./scripts/run.sh up`이다.
- `./scripts/dev.sh`는 `run.sh`로 전달하는 transition-period compatibility wrapper다.
