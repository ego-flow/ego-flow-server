...
`stream-not-ready` hook은 publisher connection 종료를 확정하고 `RecordingSession`을 `CLOSED`로 닫는다. segment complete hook은 session에 귀속된 단일 raw segment 저장 완료를 확정한다. backend는 `RecordingSegment`를 `WRITE_DONE`으로 저장하고, session이 이미 `CLOSED`이며 단일 segment가 `WRITE_DONE`일 때 finalize job을 enqueue한다. worker는 session status와 기존 Video row를 다시 검증하지 않고, `WRITE_DONE -> PROCESSING` atomic claim에 성공한 worker만 후처리를 수행한다. claim에 성공하면 expensive processing 전에 `Video.status = PROCESSING` row를 먼저 upsert해서 dashboard가 처리 중인 항목을 볼 수 있게 한다. 최종 video 생성이 끝나면 같은 `recordingSessionId`의 `Video` row를 `COMPLETED`로 upsert하고 segment를 `COMPLETED`로 전환한다.
...
flowchart TD
    ...
    Claim --> ProcessingVideo["Video PROCESSING row upsert"]
    ProcessingVideo --> Merge["단일 raw segment 선택"]
    Merge --> Metadata["ffprobe technical metadata 추출"]
    Metadata --> Encode["인코딩 3종 생성"]
    Encode --> Files["generated files 저장"]
    Files --> DB["Video COMPLETED upsert + SemanticMetadata PENDING + Segment COMPLETED"]
```
...
```mermaid
flowchart TD
    A["Job received"] --> B["RecordingSession metadata 조회"]
    B --> C["단일 RecordingSegment 조회"]
    C --> G["WRITE_DONE -> PROCESSING claim"]
    G --> P["Video PROCESSING row upsert"]
    P --> F["waitForStableFile(raw)"]
    F --> H["ffprobe technical metadata 추출"]
    H --> I["output path 계산"]
    I --> J["출력 디렉토리 생성"]
    J --> K["병렬 인코딩"]
    K --> K1["VLM mp4"]
    ...
    K4 --> L["Video row upsert"]
    K2 --> L
    K3 --> L
    L --> S["same Video row COMPLETED upsert + VideoSemanticMetadata PENDING row upsert"]
    S --> M["RecordingSegment COMPLETED"]
    M --> N["optional raw delete"]
```

Worker progress는 임의 percentage가 아니라 task step으로 저장한다. BullMQ `job.progress`의 API 노출 shape는 다음과 같다.
...
| step | task | label |
| --- | --- | --- |
| 1/7 | `initialize_video` | `Initialize video` |
| 2/7 | `stabilize_raw` | `Stabilize raw` |
| 3/7 | `probe_metadata` | `Probe metadata` |
| 4/7 | `prepare_outputs` | `Prepare outputs` |
| 5/7 | `encode_assets` | `Encode assets` |
| 6/7 | `verify_artifact` | `Verify artifact` |
| 7/7 | `persist_video` | `Persist video` |
...
worker는 finalize job을 내부 queue contract로 취급한다. enqueue 시점에 이미 `RecordingSession.status = CLOSED`와 단일 `RecordingSegment.status = WRITE_DONE` readiness check가 끝났으므로, worker는 session status나 segment status를 별도 분기로 다시 검증하지 않는다. race 방지는 segment의 `WRITE_DONE -> PROCESSING` atomic claim이 담당한다.

claim에 성공한 worker는 같은 transaction 이전에 `recordingSessionId` unique key 기준으로 `Video.status = PROCESSING` row를 upsert한다. 이 row에는 repository/session context, raw path, recorder, fallback `recordedAt`, `processingStartedAt`이 들어간다. retry가 발생해도 새 Video row를 만들지 않고 같은 `recordingSessionId` row를 다시 `PROCESSING`으로 재사용한다.
...
### 2.4 Semantic metadata 분리

현재 finalize worker는 VLM 연구용 mp4 파일을 생성하지만, VLM/semantic metadata를 생성하거나 모델을 호출하지 않는다. `Video.status = PROCESSING`은 generated artifacts 생성 중임을 뜻하고, `COMPLETED`는 재생/다운로드 가능한 generated artifacts가 준비되었다는 뜻이다. semantic metadata 완료 여부를 의미하지 않는다.

semantic metadata는 `video_semantic_metadata` 테이블에서 별도 lifecycle로 관리한다. finalize 성공 transaction은 해당 video의 semantic metadata row를 `PENDING`으로 준비한다. 이후 model 또는 heuristic을 붙일 때는 별도 semantic metadata worker가 completed video artifact를 읽고, 별도 transaction에서 아래 필드를 갱신한다.
...
- `Video` 메타데이터 필드
- `VideoSemanticMetadata.status = PENDING`
- 성공/실패 여부는 `Video.status`와 `Video.errorMessage`가 표현한다.
- 성공 시 처리 대상 `RecordingSegment.status = COMPLETED`

처리 시작 시 `Video`는 `recordingSessionId` unique key 기준으로 `PROCESSING` upsert된다. 성공 시 같은 row를 `COMPLETED`로 upsert한다. 기존 Video row 존재 여부를 사전에 조회하지 않고, 마지막 DB transaction에서 artifact metadata를 확정한다.
...
실패 시 `Video`도 `recordingSessionId` unique key 기준으로 upsert한다.
...
repository 삭제는 deactivate, readiness check, permanent delete 순서로 진행한다. permanent delete 시 backend는 다음 순서로 정리한다.

1. repository가 `deactivated=true`이고 `STREAMING` recording session과 `WRITE_DONE/PROCESSING` recording segment가 없는지 확인
2. 관련 raw/generated file 삭제
3. `repo_members`, `videos`, `recording_segments`, `recording_sessions`, `repository` DB row 삭제

즉 repository 삭제는 메타데이터 삭제만이 아니라 저장된 파일 정리까지 포함한다.

운영상 주의:

- repository rename/permanent delete는 파일 시스템 변경을 동반하므로 data retention 정책과 함께 검토해야 한다.
