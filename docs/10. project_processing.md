...
`stream-not-ready` hook은 publisher connection 종료를 확정하고 `RecordingSession`을 `CLOSED`로 닫는다. segment complete hook은 session에 귀속된 단일 raw segment 저장 완료를 확정한다. backend는 `RecordingSegment`를 `WRITE_DONE`으로 저장하고, session이 이미 `CLOSED`이며 단일 segment가 `WRITE_DONE`일 때 finalize job을 enqueue한다. worker는 session status와 기존 Video row를 다시 검증하지 않고, `WRITE_DONE -> PROCESSING` atomic claim에 성공한 worker만 후처리를 수행한다. 최종 video 생성이 끝나면 `Video`를 upsert하고 segment를 `COMPLETED`로 전환한다.
...
flowchart TD
    ...
    Files --> DB["Video upsert + SemanticMetadata PENDING + Segment COMPLETED"]
```
...
flowchart TD
    ...
    K2 --> L
    K3 --> L
    L --> S["VideoSemanticMetadata PENDING row upsert"]
...
현재 finalize worker는 VLM 연구용 mp4 파일을 생성하지만, VLM/semantic metadata를 생성하거나 모델을 호출하지 않는다. `Video.status = COMPLETED`는 재생/다운로드 가능한 generated artifacts가 준비되었다는 뜻이며 semantic metadata 완료를 의미하지 않는다.
...
성공 시 `Video`는 `recordingSessionId` unique key 기준으로 upsert한다. 기존 Video row 존재 여부를 사전에 조회하지 않고, 마지막 DB transaction에서 artifact metadata를 확정한다.
