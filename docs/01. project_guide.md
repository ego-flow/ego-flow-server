...
- MediaMTX는 RTMP publish 요청 시 backend의 `/api/v1/auth/rtmp`를 호출해 ticket를 검증한다. HLS playback은 Caddy `forward_auth`가 `/api/v1/hls-auth`로 인증/권한 게이트를 수행한다.
- MediaMTX hook은 `stream-ready`, `stream-not-ready`, `recording-segment-create`, `recording-segment-complete`로 분리되어 recording lifecycle을 반영한다. stream path의 recordingSessionId를 기준으로 session을 복구하며, `source_id`는 authoritative key로 사용하지 않는다.
