...
- [09. project_streaming.md](./09.%20project_streaming.md): stream 등록, RTMP publish, active stream 조회, 녹화 완료 webhook 흐름
- [10. project_processing.md](./10.%20project_processing.md): worker 처리, video 상태 전이, generated file 저장 구조, target directory migration
...
| Streaming | MediaMTX가 RTMP ingest, HLS playback, recording segment 생성, external auth/webhook 호출 담당 |
| Storage | PostgreSQL은 메타데이터, Redis는 stream session + queue backend, 파일 시스템은 raw 및 generated dataset 저장 |

## 핵심 구현 포인트

- Repository 단위로 스트리밍 등록, 접근 제어, 영상 목록 조회, 생성 파일 저장이 이루어진다.
- RTMP publish URL은 `repository_name` 기준으로 생성되며, publish 전 `POST /api/v1/streams/register`가 선행되어야 한다.
- MediaMTX는 RTMP publish/read 요청 시 backend의 `/api/v1/auth/rtmp`를 호출해 JWT 기반 인증을 수행한다.
- 녹화 완료 후 MediaMTX webhook이 `/api/v1/hooks/recording-complete`를 호출하면 video row가 생성되고 BullMQ job이 enqueue된다.
...
- 실제 구현 코드: `backend/`, `frontend/`, `docker-compose.yml`, `mediamtx.yml`
