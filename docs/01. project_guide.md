...
- [11. project_deploy.md](./11.%20project_deploy.md): 단일 `run.sh` 기반 실행 방식과 서버 재기동 helper 정리
- [12. project_local_test.md](./12.%20project_local_test.md): 로컬 Docker + Android 실기기 + `adb reverse` 기준 테스트 세팅 절차
...
| 영역 | 현재 역할 |
| --- | --- |
| Backend | Express 5 + TypeScript API. JWT 인증, repository 권한 검사, MediaMTX 연동, signed `/files` 접근 제어 담당 |
...
| Streaming | MediaMTX가 RTMP/WHIP ingest, direct HLS playback, recording segment 생성, external auth/hook 호출 담당 |
| Storage | PostgreSQL은 recording/video 메타데이터, Redis는 publish ticket, recording live cache, queue backend, 파일 시스템은 raw 및 generated dataset 저장 |
...
- publish가 시작되면 MediaMTX `stream-ready` hook이 publish ticket을 consume하고 `stream:recording:{recordingSessionId}` live cache 및 active stream set을 갱신한다.
- MediaMTX는 RTMP/WHIP publish와 HLS playback 요청 시 backend의 `/api/v1/auth/mediamtx`를 호출한다. publish는 publish ticket, HLS playback은 playback ticket을 검증한다.
- MediaMTX hook은 `stream-ready`, `stream-not-ready`, `recording-segment-create`, `recording-segment-complete`로 분리되어 recording lifecycle을 반영한다. `stream-ready` 인증은 ticket 기준이며, 그 외 hook도 stream path의 recordingSessionId를 기준으로 session을 복구한다.
- RTMPS listener 설정 surface는 준비되어 있지만 publish URL은 server가 반환하지 않는다. app은 backend origin과 고정 포트 정책을 기준으로 RTMP/RTMPS URL을 조립한다.
- 최종 video 생성은 segment 단위가 아니라 `RecordingSession` 단위 finalize job으로 수행된다.
- 결과 파일은 `{TARGET_DIRECTORY}/datasets/{owner_id}/{repo_name}/` 아래에 저장된다.
...
flowchart LR
    ...
    Dashboard -->|signed /files URL| Backend
```
...
7. 실행/서버 재기동 방식은 [11. project_deploy.md](./11.%20project_deploy.md)
8. 로컬 실기기 테스트 절차는 [12. project_local_test.md](./12.%20project_local_test.md)
9. API는 Swagger UI(`/api-docs`) 또는 OpenAPI JSON(`/api/v1/openapi.json`)로 본다.

## 참고 자료

- `guide/` 아래 기존 설계/구현 참고 문서
- 실제 구현 코드: `backend/`, `frontend/`, `compose.yml`, `scripts/run.sh`, `Caddyfile`, `mediamtx.yml`
