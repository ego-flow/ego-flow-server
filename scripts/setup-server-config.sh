...
validate_postgres_password() {
  ...
  echo "POSTGRES_PASSWORD contains unsupported characters." >&2
  echo "Allowed characters: letters, digits, underscore (_), dot (.), and hyphen (-)." >&2
  echo "This avoids Docker Compose interpolation and DATABASE_URL parsing issues." >&2
  exit 1
}
...
main() {
  ...
  POSTGRES_PASSWORD="$(ask_secret_with_default "POSTGRES_PASSWORD" "postgres" "PostgreSQL password; allowed: A-Z a-z 0-9 _ . -")"
  validate_postgres_password "$POSTGRES_PASSWORD"
