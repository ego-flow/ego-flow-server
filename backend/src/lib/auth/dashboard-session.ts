...
export const createDashboardSession = async (userId: string, rememberMe: boolean) => {
...
export const verifyDashboardSession = async (
  rawToken: string,
): Promise<({ sessionId: string } & AuthenticatedUser) | null> => {
  if (
  ...
  if (session.expiresAt <= Date.now()) {
    void revokeDashboardSession(rawToken);
    return null;
  }
...
export const revokeDashboardSession = async (rawToken: string) => {
