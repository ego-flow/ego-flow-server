import path from "path";

import {
  DEFAULT_RTMPS_CERT_PATH,
  DEFAULT_RTMPS_ENCRYPTION_MODE,
  DEFAULT_RTMPS_KEY_PATH,
  DEFAULT_WHIP_PATH_PREFIX,
  FIXED_PUBLIC_HTTP_PORT,
  FIXED_HLS_PORT,
  FIXED_MEDIAMTX_API_PORT,
  FIXED_MEDIAMTX_API_URL,
  FIXED_RAW_ROOT,
  FIXED_REDIS_URL,
  FIXED_RTMP_PORT,
  FIXED_RTMPS_PORT,
  TARGET_DIRECTORY_DATASETS_SUBDIRECTORY,
} from "../constants/config/config-constants";
import { fileConfig } from "./config.file";
import { env } from "./env";

export const runtimeConfig = {
  NODE_ENV: env.NODE_ENV,
  PORT: env.PORT,
  DATA_ROOT: fileConfig.TARGET_DIRECTORY,
  RAW_ROOT: FIXED_RAW_ROOT,
  TARGET_DIRECTORY: path.join(fileConfig.TARGET_DIRECTORY, TARGET_DIRECTORY_DATASETS_SUBDIRECTORY),
  PUBLIC_HTTP_PORT: FIXED_PUBLIC_HTTP_PORT,
  RTMP_PORT: FIXED_RTMP_PORT,
  RTMPS_PORT: FIXED_RTMPS_PORT,
  HLS_PORT: FIXED_HLS_PORT,
  MEDIAMTX_API_PORT: FIXED_MEDIAMTX_API_PORT,
  CORS_ORIGIN: fileConfig.CORS_ORIGIN,
  WORKER_CONCURRENCY: fileConfig.WORKER_CONCURRENCY,
  DELETE_RAW_AFTER_PROCESSING: fileConfig.DELETE_RAW_AFTER_PROCESSING,
  JWT_EXPIRES_IN: fileConfig.JWT_EXPIRES_IN,
  JWT_REFRESH_THRESHOLD_SECONDS: fileConfig.JWT_REFRESH_THRESHOLD_SECONDS,
  SIGNED_FILE_URL_EXPIRES_IN: fileConfig.SIGNED_FILE_URL_EXPIRES_IN,
  ADMIN_DEFAULT_PASSWORD: env.ADMIN_DEFAULT_PASSWORD,
  JWT_SECRET: env.JWT_SECRET,
  DATABASE_URL: env.DATABASE_URL,
  REDIS_URL: FIXED_REDIS_URL,
  HF_TOKEN: env.HF_TOKEN,
  RTMPS_ENCRYPTION_MODE: env.RTMPS_ENCRYPTION_MODE ?? DEFAULT_RTMPS_ENCRYPTION_MODE,
  RTMPS_ENABLED: (env.RTMPS_ENCRYPTION_MODE ?? DEFAULT_RTMPS_ENCRYPTION_MODE) !== DEFAULT_RTMPS_ENCRYPTION_MODE,
  RTMPS_CERT_PATH: env.RTMPS_CERT_PATH ?? DEFAULT_RTMPS_CERT_PATH,
  RTMPS_KEY_PATH: env.RTMPS_KEY_PATH ?? DEFAULT_RTMPS_KEY_PATH,
  WHIP_PATH_PREFIX: DEFAULT_WHIP_PATH_PREFIX,
  MEDIAMTX_API_URL: FIXED_MEDIAMTX_API_URL,
} as const;
