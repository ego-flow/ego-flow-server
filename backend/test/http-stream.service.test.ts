...
const { HTTP_STREAM_TIMEOUT_MS } =
  require("../src/constants/stream/stream-constants") as typeof import("../src/constants/stream/stream-constants");
...
const timedOutLastChunkAt = () => Date.now() - HTTP_STREAM_TIMEOUT_MS - 5_000;
const activeLastChunkAt = () => Date.now() - HTTP_STREAM_TIMEOUT_MS + 5_000;
...
test("reconcile leaves HTTP upload active before idle timeout", async () => {
  const rawPath = path.join(tempRoot, "recording.mp4");
  currentSession = {
    ...session,
    status: RecordingSessionStatus.STREAMING,
  ...
  assert.notEqual(await fakeRedis.get(`stream:recording:${session.id}`), null);
  assert.deepEqual(await fakeRedis.smembers("stream:active:sessions"), [session.id]);
});
...
test("reconcile does not enqueue finalize when timeout recovery loses the state claim", async () => {
  ...
  fakeRedis.setJson(`stream:recording:${session.id}`, {
    ...
    bytesReceived: 5,
    lastSequence: 0,
    lastChunkAt: timedOutLastChunkAt(),
  } satisfies RecordingSessionLiveCache);
  const enqueueCalls: string[] = [];
  recordingSessionService.tryEnqueueFinalize = async (recordingSessionId: string) => {
    enqueueCalls.push(recordingSessionId);
